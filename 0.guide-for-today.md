...
```
Click on the PR link, read it through and see if there are any errors to fix, then merge to main. You're up to date!

## Setting up for the main event

Before we launch into the longer coding session (where we build our own project), we need to set up a few additional features. We will be using a team of agents - one for planning, one for coding, and one for checking. My recommendation is that you use Opus for planning and checking, but Sonnet for coding. This will save you a lot of tokens as Sonnet works well as a coder when following instructions. 
