Create 3 global Claude Code protective hooks in ~/.claude/hooks/ for users running bypassPermissions
  ...
  - .env git-commit check: block only when both conditions are true: (a) command matches
  git\s+(add|commit) AND (b) .env appears as a standalone word — (^|\s)\.env(\s|$). This avoids           
  false-positives from exclusion pathspecs like ':!.env' where .env appears inside a larger token.
  ...
  - rm -rf — but first check if the target matches the safe allowlist                                     
  (node_modules|dist|\.next|\.turbo|build|coverage|\.cache|tmp|__pycache__); if it does, skip             
  - rm\s+-[a-zA-Z]*f\s+/(?!tmp/) — forced deletion of absolute paths outside /tmp/; use a negative        
  lookahead so rm -f /tmp/foo is allowed                                                                  
  - DROP\s+TABLE, DROP\s+DATABASE, \bTRUNCATE\b — destructive DB commands
  - kill\s+-9\s+(-1|0)\b and \bkillall\b — kill-all-processes only; do not match kill -9 <pid> (targeting 
  a specific PID is routine and legitimate)                                                               
